...
var bitcore = require('bitcore-lib-tent');
var zmq = require('zmq');
...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  var configPath = path.resolve(spawnOptions.datadir, './snowgem.conf');
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in snowgem.conf'
  );
...
Bitcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/snowgem.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/snowgem.conf';
    }
  }
  return networkPath;
};
...
Bitcoin.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting tent process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        ...
        log.warn('Restarting tent child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
